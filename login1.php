...
<?php 
 ...
 $stmt_find = $connect->prepare("SELECT * FROM `addprof`");
  $stmt_find->execute();

  while ($find_row = $stmt_find->fetch()) {
      $nomp = $find_row ['nom'];
      $password = $find_row ['pass'];}
	 


if ($_POST["usernam"] == $nomp && $_POST["pass"] == $password){
        echo "Vous avez accès au contenu protégé de cette page.";
       # redirect_to("monprofil.php");
					...
					<form class="" action="login1.php" method="post">
						<div class="form-group">
							...
							<div class="input-group mb-3">
								...
								<input type="text"  class="form-control" name="usernam" id="username" value=""> </input>
								</div>
							</div>
							<div class="form-group">
